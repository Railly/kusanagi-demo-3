...
export default function Cart() {
  const { shoppingCartVideojuego, setShoppingCartVideojuego } =
  ...
  const handleCheckout = () => {
    ...
    const messages = [
      {
        body: {
          ...
          videogames: shoppingCartVideojuego.map((item) => item.id),
          quantities: shoppingCartVideojuego.map((item) => item.quantity),
        },
    ...
    window
      .fetch("https://kusanagi-demo-3.app.vercel/api/messages", {
        method: "POST",
        headers: {
    ...
    <main className="flex items-center justify-center w-full h-screen">
      <section className="flex flex-col flex-wrap items-center w-full h-full pt-4 bg-gray-100">
        ...
        <div className="w-8/12 py-6 mt-4 overflow-y-scroll h-4/6">
          {shoppingCartVideojuego.map((libro) => (
            <div
              key={libro.id}
              ...
              <div className="flex flex-row justify-around w-full">
                ...
                <div className="flex items-center justify-between px-4">
                  <button
                    onClick={() => {
                      setShoppingCartVideojuego(
                        shoppingCartVideojuego.map((item) => {
                          if (item.id === libro.id) {
                            if (item.quantity > 0) {
